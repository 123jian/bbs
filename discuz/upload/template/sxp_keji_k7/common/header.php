	...
	<div class="cl ysection">
		...
		<!--{if $_G['uid']}-->
			<div class="has-login">
				...
				<a href="{if $_G['uid']}home.php{else}javascript:;{/if}" id="qmenu" class="" onmouseover="showMenu(this.id);">øÏΩ›µº∫Ω</a>
				...
				<!--{/if}-->
				<!--{if check_diy_perm($topic)}--><a href="javascript:saveUserdata('diy_advance_mode', '1');openDiy();"><i class="ic21"></i>DIY…Ë÷√</a><!--{/if}-->
		...
		<!--{else}-->
			...
			<!--{if  !empty($_G['style']['extstyle'])}--><a id="sslct" href="javascript:;" onMouseOver="delayShow(this, function() {showMenu({'ctrlid':'sslct','pos':'34!'})});">{lang changestyle}</a><!--{/if}--> 
				...
				<!--{/loop}-->
				<a onclick="showWindow('login', this.href);return false;" href="member.php?mod=logging&amp;action=login" rel="nofollow">”√ªßµ«¬º</a>
				<a href="member.php?mod=register" rel="nofollow">◊¢≤·’À∫≈</a>
			</div>
		<!--{/if}-->
		</div>
